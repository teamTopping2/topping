...
public class DiaryDao {
	...
	public Diary selectdiary(Connection conn, int diaryNum) {
		...
		String query = "SELELCT * FROM TB_DIARY WHERE DIR_NO = ?";
		...
		try {
			...
			if(rset.next()) {
				...
				diary.setDiaryTitle(rset.getString("diary_title"));
				diary.setUserNo(rset.getInt("user_No"));
				...
				diary.setDiaryReadCount(rset.getInt("diary_readcount"));
				diary.setRegId(rset.getString("reg_id"));
				diary.setRegCateNo(rset.getInt("reg_cate_no"));
			}
	...
	public int insertOriginDiary(Connection conn, Diary diary) {
		...
		try {
			...
			pstmt.setInt(2, diary.getUserNo());
