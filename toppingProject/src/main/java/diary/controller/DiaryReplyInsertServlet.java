...
/**
 * Servlet implementation class DiaryReplyInsertServlet
 */
@WebServlet("/dreply")
public class DiaryReplyInsertServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		Diary origin = dservice.selectDiary(diaryNum);
...
//		bservice.updateReplySeq(reply);  //update 로 1증가 처리
		...
		//3.
		int result = dservice.insertReplyDiary(reply);
		
		if(result > 0) {
			response.sendRedirect("/diary/dlist?page=" + currentPage);
		}else {
			RequestDispatcher view = request.getRequestDispatcher("views/common/error.jsp");
			request.setAttribute("message", 
					diaryNum + "번 일기에 댓글 등록을 실패하셨습니다");
